<?php
    ...
    try {
        ...
        $stmt_sekretaris_desa  = mysqli_stmt_init($connection);
        $query_sekretaris_desa = "UPDATE tbl_pegawai SET
            id_jabatan = ?
            , id_pangkat_golongan = ?
            , id_pendidikan = ?
        ...
        if (!mysqli_stmt_prepare($stmt_sekretaris_desa, $query_sekretaris_desa)) {
            $_SESSION['msg'] = 'Statement Pegawai preparation failed: ' . mysqli_stmt_error($stmt_sekretaris_desa);
            echo "<meta http-equiv='refresh' content='0;profil.php?go=profil'>";
            return;
        }
        
        mysqli_stmt_bind_param($stmt_sekretaris_desa, 'iiiisssii', $id_jabatan, $id_pangkat_golongan, $id_pendidikan, $id_jurusan, $alamat, $tmp_lahir, $tgl_lahir, $tahun_ijazah, $id_pegawai);
        
        if (!mysqli_stmt_execute($stmt_sekretaris_desa)) {
            $_SESSION['msg'] = 'Statement Pegawai preparation failed: ' . mysqli_stmt_error($stmt_sekretaris_desa);
            echo "<meta http-equiv='refresh' content='0;profil.php?go=profil'>";
            return;
        }
    ...
    mysqli_stmt_close($stmt_sekretaris_desa);
