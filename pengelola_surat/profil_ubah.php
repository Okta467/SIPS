<?php
    ...
    try {
        ...
        $stmt_pengelola_surat  = mysqli_stmt_init($connection);
        $query_pengelola_surat = "UPDATE tbl_pegawai SET
            id_jabatan = ?
            , id_pangkat_golongan = ?
            , id_pendidikan = ?
        ...
        if (!mysqli_stmt_prepare($stmt_pengelola_surat, $query_pengelola_surat)) {
            $_SESSION['msg'] = 'Statement Pegawai preparation failed: ' . mysqli_stmt_error($stmt_pengelola_surat);
            echo "<meta http-equiv='refresh' content='0;profil.php?go=profil'>";
            return;
        }
        
        mysqli_stmt_bind_param($stmt_pengelola_surat, 'iiiisssii', $id_jabatan, $id_pangkat_golongan, $id_pendidikan, $id_jurusan, $alamat, $tmp_lahir, $tgl_lahir, $tahun_ijazah, $id_pegawai);
        
        if (!mysqli_stmt_execute($stmt_pengelola_surat)) {
            $_SESSION['msg'] = 'Statement Pegawai preparation failed: ' . mysqli_stmt_error($stmt_pengelola_surat);
            echo "<meta http-equiv='refresh' content='0;profil.php?go=profil'>";
            return;
        }
    ...
    mysqli_stmt_close($stmt_pengelola_surat);
