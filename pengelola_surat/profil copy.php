  ...
  <body class="nav-fixed">
              ...
              <div class="card-body">
                <table id="datatablesSimple">
                  ...
                  <tbody>
                    ...
                    <?php
                    $no = 1;
                    $query_pengelola_surat = mysqli_query($connection, 
                      "SELECT
                      ...
                      WHERE a.id = {$_SESSION['id_pegawai']}");

                    while ($pengelola_surat = mysqli_fetch_assoc($query_pengelola_surat)):
                    ?>

                      <tr>
                        <td><?= $no++ ?></td>
                        <td>
                          <?= htmlspecialchars($pengelola_surat['nama_pegawai']); ?>
                          <?= !empty($pengelola_surat['nip'])
                            ? "<br><small class='text-muted'>({$pengelola_surat['nip']})</small>"
                            : "<br><small class='text-muted'>(Tidak ada NIP)</small>"; ?>
                        </td>
                        <td>
                          <div class="ellipsis toggle_tooltip" title="<?= $pengelola_surat['nama_pangkat_golongan'] ?>">
                            <?= $pengelola_surat['nama_pangkat_golongan'] ?>
                          </div>
                        </td>
                        <td>
                          <div class="text-center"><?= strtoupper($pengelola_surat['tipe_pangkat_golongan']) ?></div>
                        </td>
                        <td>
                          <?= "{$pengelola_surat['nama_pendidikan']}/{$pengelola_surat['nama_jurusan_pendidikan']}/{$pengelola_surat['tahun_ijazah']}" ?>
                        </td>
                        <td>
                          <div class="ellipsis toggle_tooltip" title="<?= $pengelola_surat['nama_jabatan'] ?>">
                            <?= $pengelola_surat['nama_jabatan'] ?>
                          </div>
                        </td>
                        <td>
                          <button class="btn btn-datatable btn-icon btn-transparent-dark me-2 toggle_modal_ubah" type="button"
                            data-id_pegawai="<?= $pengelola_surat['id_pegawai'] ?>">
                            <i class="fa fa-pen-to-square"></i>
                          </button>
                          <button class="btn btn-datatable btn-icon btn-transparent-dark me-2 toggle_modal_detail_pengelola_surat" type="button"
                            data-id_pegawai="<?= $pengelola_surat['id_pegawai'] ?>"
                            data-nama_pegawai="<?= $pengelola_surat['nama_pegawai'] ?>"
                            data-username="<?= $pengelola_surat['username'] ?>"
                            data-hak_akses="<?= $pengelola_surat['hak_akses'] ?>"
                            data-alamat="<?= $pengelola_surat['alamat'] ?>"
                            data-tmp_lahir="<?= $pengelola_surat['tmp_lahir'] ?>"
                            data-tgl_lahir="<?= $pengelola_surat['tgl_lahir'] ?>">
        ...
        <div class="modal-content">
          ...
          <form>
            <div class="modal-body">
              ...
              <div class="mb-3">
                <label class="small mb-1" for="xnama_pegawai">Nama Guru <span class="text-danger fw-bold">*</span></label>
                <input class="form-control" id="xnama_pegawai" type="text" name="xnama_pegawai" placeholder="Enter nama pengelola_surat" disabled>
              </div>
              ...
              <div class="mb-3">
                ...
                <select name="xhak_akses" class="form-control select2" id="xhak_akses" disabled>
                  <option value="pengelola_surat" selected>Sekretaris Desa</option>
                </select>
              </div>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('.toggle_modal_ubah').on('click', function() {
          ...
          $.ajax({
            ...
            success: function(data) {
              $('#ModalInputProfil #xid_pegawai').val(data[0].id_pegawai);
              $('#ModalInputProfil #xid_pengguna').val(data[0].id_pengguna);
              $('#ModalInputProfil #xnip').val(data[0].nip);
              $('#ModalInputProfil #xnama_pegawai').val(data[0].nama_pegawai);
          ...
          $('#ModalInputProfil #xid_pegawai').val(id_pegawai);
          $('#ModalInputProfil #xnama_pegawai').val(nama_pegawai);
        ...
        $('.toggle_modal_detail_pengelola_surat').on('click', function() {
          const data = $(this).data();
        
          $('#ModalDetailProfil .xnama_pegawai').html(data.nama_pegawai);
